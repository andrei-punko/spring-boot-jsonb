...
class ArticleServiceTest {
    ...
    void getAllByLocation() {
        ...
        final List<Article> articles = List.of(new Article());
        final List<ArticleDto> articleDtoList = List.of(new ArticleDto());
        when(articleRepositoryMock.getArticleByCountryNCity(country, city)).thenReturn(articles);
        when(articleMapperMock.toArticleDtoList(articles)).thenReturn(articleDtoList);

        List<ArticleDto> result = articleService.getArticleByLocation(country, city);
